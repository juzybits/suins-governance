...
export function ProposalContent({ proposalId }: { proposalId: string }) {
  return (
    <div>
      <VoteBg />
      <div className="mx-auto mt-5xl flex w-full max-w-[88rem] flex-col gap-3xl px-l xl:flex-row">
        <div className="flex w-full flex-col items-center justify-center gap-3xl sm:gap-4xl md:basis-2/3">
          <ProposalText proposalId={proposalId} />
          <CastYourVoteV2 proposalId={proposalId} />
          <Votes proposalId={proposalId} />
        </div>
        <div className="hidden flex-col gap-xl md:sticky md:h-[987px] md:min-h-fit md:basis-1/3 md:pt-10 xl:sticky xl:top-0 xl:flex">
